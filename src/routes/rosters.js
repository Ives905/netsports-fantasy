...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Get tiebreakers
    const tiebreakersResult = await pool.query(`
      SELECT round, question1_answer, question2_answer
      FROM tiebreakers WHERE user_id = $1
    `, [req.user.id]);
...
router.post('/:round/submit', authenticateToken, async (req, res) => {
  try {
    ...
    // Submit roster
    await pool.query(`
      UPDATE rosters SET is_submitted = true, submitted_at = NOW()
      WHERE id = $1
    `, [rosterId]);
